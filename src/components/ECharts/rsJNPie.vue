<template>
  <div id="rsPie" :style="{ width: '100%', height: '100%' }"></div>
</template>

<script>
import { RsZyzgZs, RsZyzgZs2 } from '../../api/api'
...
export default {
  ...
  methods: {
    onSHow () {
      ...
      setTimeout(() => {
        ...
        if (this.path === '/rs-primaryIndex') {
          ...
          RsZyzgZs2(this.$store.state.fgsSelValue).then((res) => {
            if (res.data.code === 0) {
              res.data.data.forEach((item) => {
                if (item.gradeName != null) {
                  let obj = {
    ...
    drawLine () {
      ...
      let myChart = echarts.init(document.getElementById('rsPie'))
      ...
      myChart.setOption({
        ...
        //   orient: 'horizontal',
        //   // x: 'left',
        //   data: ['初级工(五级)', '中级工(四级)', '高级工(三级级)', '技师(二级)', '高级技师(一级)'],
        //   right: '25%',
        //   bottom: '1%',
        //   align: 'right',
        //   textStyle: {
